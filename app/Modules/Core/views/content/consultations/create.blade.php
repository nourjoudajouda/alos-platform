...
@section('title', __('Add Consultation') . ' — ' . config('app.name'))

@section('content')
<div class="container-xxl flex-grow-1 container-p-y" dir="{{ $contentDir }}">
  <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <h4 class="fw-bold mb-0">{{ __('Add Consultation') }}</h4>
    <a href="{{ route(($consultationRoutePrefix ?? 'admin.core.consultations') . '.index') }}" class="btn btn-outline-secondary btn-sm">
  ...
  <div class="card">
    <div class="card-body">
      <form action="{{ route(($consultationRoutePrefix ?? 'admin.core.consultations') . '.store') }}" method="post">
        @csrf
        <div class="row g-3">
          <div class="col-md-6">
          ...
          <div class="col-md-6">
            ...
            <input type="date" name="consultation_date" id="consultation_date" class="form-control @error('consultation_date') is-invalid @enderror" value="{{ old('consultation_date', now()->format('Y-m-d')) }}" required>
          ...
          <div class="col-12">
            ...
            <textarea name="summary" id="summary" class="form-control @error('summary') is-invalid @enderror" rows="3" maxlength="5000">{{ old('summary') }}</textarea>
          ...
          <div class="col-12">
            ...
            <textarea name="internal_notes" id="internal_notes" class="form-control @error('internal_notes') is-invalid @enderror" rows="2" maxlength="5000">{{ old('internal_notes') }}</textarea>
          ...
          <div class="col-12">
            <div class="form-check">
              <input type="checkbox" name="is_shared_with_client" id="is_shared_with_client" value="1" class="form-check-input" {{ old('is_shared_with_client') ? 'checked' : '' }}>
              <label for="is_shared_with_client" class="form-check-label">{{ __('Share with client in portal') }}</label>
            </div>
          </div>
        </div>
        <div class="mt-4 d-flex gap-2">
          <button type="submit" class="btn btn-primary">{{ __('Create Consultation') }}</button>
          <a href="{{ route(($consultationRoutePrefix ?? 'admin.core.consultations') . '.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
        </div>
      </form>
    </div>
  </div>
