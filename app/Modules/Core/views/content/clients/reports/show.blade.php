...
<div class="container-xxl flex-grow-1 container-p-y" dir="{{ $contentDir }}">
  <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-1">
          ...
          <li class="breadcrumb-item"><a href="{{ route('admin.core.clients.reports.index', $client) }}">{{ __('Reports') }}</a></li>
    ...
    <a href="{{ route('admin.core.clients.reports.index', $client) }}" class="btn btn-outline-secondary btn-sm">{{ __('Back to reports') }}</a>
  </div>
  ...
  <div class="card">
    <div class="card-body">
      ...
      @elseif(($payload['report_type'] ?? '') === 'new_documents' && !empty($payload['documents']))
        <h6 class="mb-3">{{ __('Documents shared') }}</h6>
        <div class="table-responsive">
          <table class="table table-bordered table-sm">
            <thead>
              <tr>
