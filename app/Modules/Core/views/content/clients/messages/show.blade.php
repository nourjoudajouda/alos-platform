...
@section('title', $thread->subject . ' — ' . config('app.name'))
...
<div class="container-xxl flex-grow-1 container-p-y" dir="{{ $contentDir }}">
    ...
    <div>
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-1">
          ...
          <li class="breadcrumb-item"><a href="{{ route($clientRoutePrefix . '.threads.index', $client) }}">{{ __('Messages') }}</a></li>
      ...
      <div class="d-flex flex-wrap gap-1 mt-1">
        @if ($thread->archived_at)
          <span class="badge bg-label-warning">{{ __('Archived') }}</span>
        @endif
        @if ($thread->case_id && $thread->case)
          <a href="{{ route($caseRoutePrefix . '.show', $thread->case) }}" class="badge bg-label-info text-decoration-none">{{ __('Case') }}: {{ $thread->case->case_number ?? $thread->case_id }}</a>
        @endif
      </div>
    </div>
    <div class="d-flex gap-2">
      <a href="{{ route($clientRoutePrefix . '.threads.index', $client) }}" class="btn btn-outline-secondary btn-sm">{{ __('Back to list') }}</a>
        ...
        <div class="row g-2">
          @foreach ($allAttachments as $item)
            ...
            <div class="col-12 col-sm-6 col-md-4">
              <a href="{{ route($clientRoutePrefix . '.threads.attachments.download', [$client, $thread, $att]) }}" class="d-flex align-items-center gap-2 p-2 rounded bg-light text-body text-decoration-none border border-secondary" target="_blank" rel="noopener">
          ...
          <div class="d-flex gap-3 {{ $msg->isFromClient() ? ($contentDir === 'rtl' ? 'flex-row-reverse' : '') : '' }}">
            ...
            <div class="flex-grow-1 min-w-0 {{ $msg->isFromClient() ? 'text-end' : '' }}">
              <div class="d-flex align-items-center gap-2 small mb-1 {{ $msg->isFromClient() ? 'justify-content-end' : '' }}">
                ...
                @if ($msg->isFromClient())
                  <span class="badge bg-label-info">{{ __('Client') }}</span>
                @else
              ...
              @if ($msg->attachments->isNotEmpty())
                <div class="mt-2 small">
                  ...
                  @foreach ($msg->attachments as $att)
                    <a href="{{ route($clientRoutePrefix . '.threads.attachments.download', [$client, $thread, $att]) }}" class="d-inline-flex align-items-center gap-1 rounded px-2 py-1 bg-white border border-secondary text-body text-decoration-none {{ $contentDir === 'rtl' ? 'ms-1' : 'me-1' }} mb-1" target="_blank" rel="noopener" title="{{ $att->name }}">
  ...
  @if (!$thread->archived_at)
    ...
    <div class="card">
      ...
      <div class="card-body">
        <form action="{{ route($clientRoutePrefix . '.threads.messages.store', [$client, $thread]) }}" method="post" enctype="multipart/form-data">
          @csrf
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <div class="form-text small">{{ __('Uploaded files will appear under each message and in the "Attachments in this conversation" section at the top.') }}</div>
